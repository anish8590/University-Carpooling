...
public class Driver extends Passenger {
    ...
    public ArrayList<SingleRide> viewSingleOfferedRides() {
        ...
        try {
            ...
            //crs.setCommand("Select * from offered_rides where driver_id = ?");
            ...
            while (crs.next()) {
                ...
                try {
                    ...
                    crs2.setCommand("Select * from offered_single_rides where ride_id = " + crs.getString("ride_id"));
                    crs2.execute();
                    while (crs2.next()) {
    ...
    public boolean updateDriverProfile(String email, String carModel, Integer capacity, Part eid, Part license, Part carReg) throws SQLException {
        try {
            ...
            if (eidIS != null && licenseIS != null && carIS != null) {
                ...
                crs.execute();
               // EmailSender.notifyAdminNewApplicant("www.anish2k2@gmail.com", email); //The actual admin email address is invalid, hence we use this as a trial
            } else {
                return false;
            }
