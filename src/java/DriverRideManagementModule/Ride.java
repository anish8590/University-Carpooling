...
public abstract class Ride {
    ...
    public boolean updateRideInfo(){
        try {
            ...
            Integer first = (int) Math.abs(first4);
            
            CachedRowSet crs2 = CarpoolDatabase.DbRepo.getConfiguredConnection();
            crs2.setCommand("INSERT INTO OFFERED_RIDES (ride_id,driver_id, is_to_uni, arrival_dep_time, start_location, end_location, current_seat_avail) VALUES (?,?,?,?,?,?,?)");
            crs2.setInt(1,first);
            this.setRideId(first);
            crs2.setString(2,driver.getEmailID());
            crs2.setString(3,isToUni ? "1" : "0");
            ...
            crs2.setTimestamp(4, ts);
            
            crs2.setString(5, startingLocation.toDbString());
            crs2.setString(6, endingLocation.toDbString());
            crs2.setInt(7,crs1.getInt("car_capacity"));
